...
@click.option(
    "--config", "-c", help="Path to config YAML file (for dataset name in logging)"
)
def main(
    model: str,
    output: str,
    ...
    if not onnx and not torchscript:
        print(
            "ERROR: Please specify at least one export format: --onnx or --torchscript"
        )
        sys.exit(1)
    ...
    if onnx:
        ...
        try:
            ...
            logger.info(
                f"  File size: {onnx_path.stat().st_size / (1024 * 1024):.2f} MB"
            )
    ...
    if torchscript:
        logger.info(
            f"  TorchScript: torch.jit.load('{output_dir}/{model_type}_model_scripted.pt')"
        )
    if onnx:
        logger.info(
            f"  ONNX: import onnx; model = onnx.load('{output_dir}/{model_type}_model.onnx')"
        )
    logger.info("=" * 60)
