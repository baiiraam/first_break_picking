...
def main(model, input, output, config, device, batch_size, chunk_size):
    ...
    if model.startswith("models:/"):
        ...
        model_obj = create_model(cfg.model_name, in_channels=1, out_channels=3)
    ...
    for i in range(0, n_traces, chunk_size):
        ...
        chunk = torch.from_numpy(chunk).float().unsqueeze(1).to(device)
        
        with torch.no_grad():
            output = model_obj(chunk)
        ...
        if (i + chunk_size) % (chunk_size * 10) == 0:
            logger.info(f"⏳ Processed {min(i+chunk_size, n_traces)}/{n_traces} traces")
    ...
    if output.endswith('.npy'):
        ...
        import pandas as pd
        df = pd.DataFrame({
            'trace_idx': range(len(all_picks)),
            'pick_sample': all_picks
        })
