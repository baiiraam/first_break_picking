...
class SweepExperiment:
    ...
    def run_experiment(
        ...
        try:
            ...
            # Run training with explicit check=False
            ...
            if self.mlflow_manager:
                ...
                self.mlflow_manager.log_artifact(
                    "configs/batch_config.yaml", artifact_path="configs"
                )
        ...
        # Remove blind Exception catch - all expected cases are handled above
    ...
    def build_command(
        self, dataset: str, model: str, loss: str, loss_params: dict
    ) -> list:
        ...
        cmd = [
            ...
            model,
            "--epochs",
            str(self.global_config.get("epochs", 2)),
            "--loss",
            loss,
            "--device",
            self.global_config.get("device", "mps"),
        ]
        ...
        def safe_parse(line: str, prefix: str, key: str) -> None:
            ...
            if prefix in line:
                try:
                    ...
                    # Log at debug level since this is expected for some lines
                    logger.debug(
                        f"Could not parse {key} from line: {line[:50]}... ({e})"
                    )

        for line in output.split("\n"):
            safe_parse(line, "Train Loss:", "train_loss")
            safe_parse(line, "Val Loss:", "val_loss")
            safe_parse(line, "Train IoU:", "train_iou")
            safe_parse(line, "Val IoU:", "val_iou")
            safe_parse(line, "Train Acc:", "train_acc")
            safe_parse(line, "Val Acc:", "val_acc")
    ...
    def print_summary(self, results: list, failed: list, total: int):
        ...
        if failed:
            ...
            for f in failed[:10]:
                logger.info(
                    f"  • {f['dataset']} | {f['model']} | {f['loss']}: {f.get('error', 'Unknown')[:100]}"
                )
            if len(failed) > 10:
...
@click.command()
@click.option(
    "--config", "-c", default="configs/sweep_config.yaml", help="Sweep config file"
)
def main(config: str):
    """Run MLflow sweep."""
