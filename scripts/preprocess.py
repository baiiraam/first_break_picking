...
@click.option(
    "--force", "-f", is_flag=True, help="Force reprocessing even if chunks exist"
)
...
def main(config: str, force: bool, dataset: str):
    ...
    logger.info(
        f"Trace counts: min={trace_counts.min()}, max={trace_counts.max()}, mean={trace_counts.mean():.1f}"
    )
    ...
    logger.info(
        f"Valid shots: {len(valid_shots)} (filtered {total_shots - len(valid_shots)} with <10 traces)"
    )
    ...
    for split_name, chunk_list in chunks.items():
        for chunk in tqdm(chunk_list, desc=f"Processing {split_name}"):
            ...
            data_batch = np.zeros(
                (n_shots, cfg.target_traces, cfg.n_samples), dtype=np.float32
            )
            mask_batch = np.zeros(
                (n_shots, cfg.target_traces, cfg.n_samples), dtype=np.int64
            )
            ...
            for i, shot_id in enumerate(shot_ids):
                ...
                shot_data, shot_picks = load_shot_data(
                    cfg.hdf5_path, start_idx, end_idx, cfg.target_traces, cfg.n_samples
                )
