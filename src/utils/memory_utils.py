...
def log_memory_stats(prefix: str = ""):
    ...
    logger.info(f"  System: {usage['system']['used_gb']:.1f}GB / {usage['system']['total_gb']:.1f}GB ({usage['system']['percent']}%)")
    
    if "cuda" in usage:
        logger.info(f"  CUDA: {usage['cuda']['allocated_gb']:.2f}GB allocated, {usage['cuda']['reserved_gb']:.2f}GB reserved")
    
    if "mps" in usage:
        logger.info(f"  MPS: {usage['mps']['allocated_gb']:.2f}GB allocated, {usage['mps']['driver_gb']:.2f}GB driver")