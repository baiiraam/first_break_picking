...
class TensorBoardManager:
    ...
    def log_image(self, tag: str, image: np.ndarray, step: Optional[int] = None, dataformats: str = "HWC"):
        """Log an image."""
        if step is None:
            step = self.step
        self.writer.add_image(tag, image, step, dataformats=dataformats)
    
    def log_figure(self, tag: str, figure: plt.Figure, step: Optional[int] = None):
    ...
    def log_seismogram(
        ...
        # Original seismogram
        ax = axes[0]
        ax.imshow(data.T, cmap='seismic', aspect='auto',
    ...
    def log_loss_curves(self, train_losses: List[float], val_losses: List[float], step: int):
        """Log loss curves."""
    ...
    def log_weights_histograms(self, model: torch.nn.Module, step: Optional[int] = None):
