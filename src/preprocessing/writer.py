...
class ChunkWriter:
    ...
    def write_chunk(
        ...
        chunk_data = {
            ...
            'split_idx': torch.tensor([0 if split == 'train' else 1 if split == 'val' else 2]),
    ...
    def verify_chunk(self, filepath: Path) -> bool:
        ...
        try:
            ...
            required_keys = ['data', 'mask', 'shot_ids', 'split_idx', 'chunk_id', 'n_shots']
            ...
            if chunk['data'].shape[0] != chunk['n_shots'].item():
                logger.error(f"Data shape mismatch in {filepath}: data has {chunk['data'].shape[0]} shots, expected {chunk['n_shots'].item()}")
                return False
            ...
            if self.compute_checksums:
                ...
                if checksum_path.exists():
                    ...
                    if stored_checksum != computed_checksum:
                        logger.error(f"Checksum mismatch in {filepath}: stored={stored_checksum}, computed={computed_checksum}")
                        return False
