...
class LRUCache:
    ...
    def put(self, key: int, value: Dict[str, Any]):
        ...
        logger.debug(f"[Cache] PUT key={key} | size={len(self.cache)}/{self.max_size} | active_keys={list(self.cache.keys())}")
        ...
        if len(self.cache) >= self.max_size:
            ...
            logger.info(f"[Cache] EVICT key={oldest_key} | size={len(self.cache)}/{self.max_size} (full)")
