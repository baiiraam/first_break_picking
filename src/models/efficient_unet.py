...
class EfficientUNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        d8 = self.dec8(e8)
        e7_up = F.interpolate(e7, size=d8.shape[2:], mode='bilinear', align_corners=False)
        d8 = torch.cat([d8, e7_up], dim=1)   # 320+320=640
        
        d7 = self.dec7(d8)   # 640 → 192
        e6_up = F.interpolate(e6, size=d7.shape[2:], mode='bilinear', align_corners=False)
        d7 = torch.cat([d7, e6_up], dim=1)   # 192+192=384
        
        d6 = self.dec6(d7)   # 384 → 112
        e5_up = F.interpolate(e5, size=d6.shape[2:], mode='bilinear', align_corners=False)
        d6 = torch.cat([d6, e5_up], dim=1)   # 112+112=224
        
        d5 = self.dec5(d6)   # 224 → 80
        e4_up = F.interpolate(e4, size=d5.shape[2:], mode='bilinear', align_corners=False)
